        ...
        View.OnClickListener saveMessage = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!messageText.isEmpty() && !recipientText.isEmpty()) {
                    MessageManager.getInstance(AddMessagesActivity.this).addMessage(new Message(recipientText, messageText, LocalDateTime.now()));

                    Toast.makeText(getApplicationContext(), "Sending now", Toast.LENGTH_SHORT).show();

                    MessageWidgetProvider.updateTextWidgets(getApplicationContext(), messageText,recipientText);
...
//                MessageWidgetProvider.updateTextWidgets(getApplicationContext(), messageFromIntent,recipientFromIntent);
            }
        }
    }
}
